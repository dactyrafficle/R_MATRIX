/* LAST UPDATED : 2022-12-04-1111 EST */
...
rafficot.get_mixed_strategy_coefficients = function(matrix_A__) {
  ...
  let mu_q_ = 1 / n_cols_;
    ...
    function get_variance_of_conditional_expected_values(A, q) {
      let EV = rafficot.get_product(A, q);
    ...
    function get_new_probability_vector(A, p, k) {
      ...
      let dp = rafficot.get_clone(p);
      for (let i = 0; i < p.length; i++) {
      
        let p_ = rafficot.get_clone(p);
        p_[i][0] += k;
      ...
      p = rafficot.get_difference(p, dp);
    ...
    // obj.ROW.CONDITIONAL_EXPECTED_VALUE = rafficot.get_product(obj.ROW.PAYOFF_MATRIX, obj.q);
